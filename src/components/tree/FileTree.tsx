...
const ROOT_DROP_ID = "xuro:notes-root";
...
function Row({ node, depth }: { node: TreeNode; depth: number }) {
  ...
  const isActive =
    (view?.type === "note" || view?.type === "canvas") && view.rel === node.rel;
    ...
    <>
      <div
        ...
        className={cx(
          ...
          isActive
            ? "bg-active text-ink before:absolute before:inset-y-1.5 before:left-0 before:w-[2.5px] before:rounded-full before:bg-accent-indicator"
        ...
        onClick={() => {
          ...
          if (isFolder) {
            ...
            setView(
              node.kind === "canvas"
                ? { type: "canvas", rel: node.rel }
                : { type: "note", rel: node.rel },
            );
          }
        }}
        onContextMenu={(event) => {
