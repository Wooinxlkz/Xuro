...
export function entryMenuItems(
  ...
  const locks = useLocks.getState();
  const locked = locks.isLocked(node.rel);
  ...
  items.push({
    ...
    icon: Lock,
    onSelect: () =>
      locks.openDialog({ rel: node.rel, kind: node.kind, mode: "manage" }),
  });
  if (locked) {
    items.push({
      label: "Remove PIN",
      icon: LockOpen,
      onSelect: () =>
        locks.openDialog({ rel: node.rel, kind: node.kind, mode: "remove" }),
    });
  }
