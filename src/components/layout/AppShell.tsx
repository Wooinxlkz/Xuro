...
import { cx, isMac, noteTitle } from "@/lib/utils";
import {
  ActionSwapIcon,
  ActionSwapText,
...
function viewPath(
  ...
  if (view.type === "todos") return `${root}/.xuro/todos.json`;
  if (view.type === "bookmarks") return `${root}/.xuro/bookmarks.json`;
  return null;
}
...
export function AppShell() {
  ...
  const [templatePicker, setTemplatePicker] = useState<MenuPosition | null>(null);
  ...
  useEffect(() => {
    ...
    window.addEventListener("xuro:cloud-account", update);
    return () => window.removeEventListener("xuro:cloud-account", update);
  }, []);
  ...
  useEffect(() => {
    ...
    window.addEventListener("xuro:publish-status", update);
    return () => window.removeEventListener("xuro:publish-status", update);
  }, []);
    ...
    <div className="flex h-full bg-bg">
      ...
      <main className="relative flex min-w-0 flex-1 flex-col">
        {/* Row 1 — titlebar: sidebar toggle + open-note tabs. The strip is
            ...
            as merged with the pane below, code-editor style. Hidden
        ...
        {/* Row 2 — current view title + right-side actions. Hidden in Zen
            mode along with Row 1. */}
        {!zenMode && (
                  ...
                  >
                    ...
                    <NoteMenuButton
                      ...
                      onClick={(event) => {
                        ...
                        setTemplatePicker({ x: rect.right + 6, y: rect.top });
                      }}
                    />
                    <NoteMenuButton
                      icon={Copy}
              ...
              templates.length > 0
                ? templates.map((template) => ({
                    ...
                    onSelect: async () => {
                      ...
                      try {
                        const content = await ipc.templateResolve(
                          template.id,
                          noteTitle(view.rel),
                        );
                        dispatchNoteAction("insert-template", content);
                      } catch (err) {
                        toast.error(
                ...
                : [
                    {
                      label: "No templates yet — open Templates to add one",
                      onSelect: () => useVault.getState().setView({ type: "templates" }),
                    },
                  ]
            }
          />
        ...
        <div className="relative min-h-0 flex-1">
          {zenMode && (
            <Tooltip label="Exit Zen mode (Esc)" side="bottom">
              <button
                type="button"
                onClick={toggleZenMode}
                className="absolute right-3 top-3 z-20 grid h-7 w-7 place-items-center rounded-md text-faint opacity-40 transition-opacity duration-150 hover:bg-hover hover:text-ink hover:opacity-100 focus-visible:opacity-100"
              >
...
function dispatchNoteAction(action: NoteAction, payload?: string) {
  window.dispatchEvent(
    new CustomEvent("xuro:note-action", { detail: { action, payload } }),
  );
}
