...
export function CloudAccountCard({
  ...
  const updateAccount = (next: CloudAccount | null) => {
    ...
    window.dispatchEvent(
      new CustomEvent("xuro:cloud-account", {
  ...
  const verifyCode = async (explicitCode?: string) => {
    if (!challenge) return;
    const submitCode = explicitCode ?? code;
    setBusy("verify");
    setError(null);
    try {
      const next = await ipc.cloudVerifyOtp(challenge.challengeId, submitCode);
      updateAccount(next);
      setStep("account");
      setChallenge(null);
      setCode("");
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="mt-0.5 text-[10.5px] text-faint">
                {account
                  ? "Publishing is connected to this Xuro account."
