...
export function ReleaseNotesModal() {
  ...
  const release = useUpdater((state) => state.release);
  ...
  return (
    <Modal
      open={open && Boolean(release)}
      onClose={dismiss}
      ariaLabel={`What's new in Xuro ${release?.version ?? ""}`.trim()}
      className="w-[460px]"
    >
      <header className="flex items-center gap-3 border-b border-line-soft px-5 py-4">
        ...
        <button
          ...
          onClick={dismiss}
          className="ml-auto grid h-8 w-8 place-items-center rounded-md text-faint transition-colors duration-100 hover:bg-hover hover:text-ink"
        >
      ...
      <div className="px-5 py-5">
        ...
        <p className="mt-3 text-[10.5px] leading-4 text-faint">
          Downloads the installer in your browser. Close Xuro, then run it to update.
        </p>
      </div>
