...
function ColorPicker({
  ...
  return (
    <div className="flex h-8 items-center gap-1 px-0.5">
      ...
      <CustomColorSwatch mode={mode} active={active} colors={colors} apply={apply} />
      <button
        type="button"
...
/**
 * Sixth swatch: opens the OS-native color picker so any hex value can be
 * applied, not just the five presets. Shows the currently-applied color
 * when it isn't one of the presets, and a neutral swatch-wheel otherwise.
 */
function CustomColorSwatch({
  ...
  const isPreset = colors.some((color) => color.value === active);
  const customValue = !isPreset && typeof active === "string" ? active : null;

  return (
    <button
      type="button"
      aria-label={`Custom ${mode === "text" ? "text" : "highlight"} color`}
      aria-pressed={Boolean(customValue)}
      className={cx(
        "relative grid h-7 w-7 place-items-center rounded-md transition-colors hover:bg-hover",
        customValue && "bg-active",
      )}
    >
      <span
        className={cx(
      ...
      <input
        type="color"
        aria-label={`Pick a custom ${mode === "text" ? "text" : "highlight"} color`}
        value={customValue ?? "#191919"}
