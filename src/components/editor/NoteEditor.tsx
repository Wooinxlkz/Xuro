...
export const NoteEditor = memo(function NoteEditor({
  ...
  const extensions = useMemo(
    () => [
      ...createExtensions(vaultRoot, () => editorRef.current),
  ...
  useEffect(() => {
    ...
    const handle = async (event: Event) => {
      try {
        const detail = (event as CustomEvent<{ action?: string; payload?: string }>)
          .detail;
        ...
        if (action === "find") {
          ...
          const content = detail?.payload;
          if (content) {
            editor.chain().focus().insertContent(content, { contentType: "markdown" }).run();
          }
        } else if (action === "export-pdf") {
          try {
            const path = await exportNoteToPdf(
              editor.view.dom as HTMLElement,
              relRef.current,
              noteTitle(relRef.current),
            );
            if (path) toast("Note exported as PDF", { description: path });
          } catch (err) {
    ...
    window.addEventListener("xuro:note-action", handle);
    return () => window.removeEventListener("xuro:note-action", handle);
  }, [
    active,
    debouncedPersist,
    editor,
