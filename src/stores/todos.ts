...
/** Matches `boards::DEFAULT_BOARD_ID` on the Rust side — the board a todo
 * lands on when nothing more specific is asked for (the flat Tasks list
 * doesn't care which board a card is filed under). */
...
interface TodosState {
  ...
  load: () => Promise<void>;
  add: (text: string, tags?: string[], boardId?: string) => Promise<void>;
...
export const useTodos = create<TodosState>((set, get) => ({
  ...
  add: async (text, tags, boardId) => {
    try {
      let todo = await ipc.todoAdd(text, boardId ?? DEFAULT_BOARD_ID);
      if (tags && tags.length) {
  ...
  setStatus: async (id, status) => {
    // optimistic — a Kanban drag must feel instant too
    set({
    ...
    try {
      const updated = await ipc.todoSetStatus(id, status);
      set({ todos: get().todos.map((t) => (t.id === id ? updated : t)) });
    } catch (err) {
      oops(err);
      get().load();
