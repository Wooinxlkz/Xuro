...
const LockDialog = lazy(() =>
  import("@/components/locks/LockDialog").then((module) => ({
    default: module.LockDialog,
  })),
);
export default function App() {
  ...
  useEffect(() => {
    if (status === "ready" && root) {
      ...
      useLocks.getState().clear();
      void useLocks.getState().load();
    } else {
      usePins.getState().clear();
      useLocks.getState().clear();
    }
  ...
  useEffect(() => {
    ...
    let disposed = false;
    void listen("xuro:notes-changed", () => {
      void refreshTree();
  ...
  useEffect(() => {
    let unlisten: (() => void) | undefined;
    let disposed = false;
    void listen<string>("xuro:tray-open-note", (event) => {
      useVault.getState().setView({ type: "note", rel: event.payload });
    }).then((stop) => {
      if (disposed) stop();
      else unlisten = stop;
    });
  ...
  useEffect(() => {
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      if (event.key === "Escape" && ui.zenMode) {
      ...
      if (matchesShortcut(event, shortcuts.dailyNote) && vault.status === "ready") {
      ...
      } else if (matchesShortcut(event, shortcuts.openDailyNotes) && vault.status === "ready") {
        event.preventDefault();
        vault.setView({ type: "daily" });
      } else if (matchesShortcut(event, shortcuts.openTags) && vault.status === "ready") {
        event.preventDefault();
        vault.setView({ type: "tags" });
        ...
        event.preventDefault();
        ui.toggleSidebar();
      } else if (matchesShortcut(event, shortcuts.toggleZenMode)) {
        event.preventDefault();
        ui.toggleZenMode();
...
function toggleSidebarEditorFocus() {
  const sidebar = document.querySelector<HTMLElement>("[data-xuro-sidebar]");
