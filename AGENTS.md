...
- `studio.rs` — Inkwell's backend: writing projects and their chapters, all in `.xuro/studio.json` (same one-JSON-store shape as `manga_online.rs`). Every project has a `kind`: Prose (chaptered writing — `content` is Markdown, matching the Notes editor's own content model) or Panel (manga/manhwa page layouts — `content` is a serialized Excalidraw scene instead). Both kinds share the exact same `Chapter` struct and every CRUD function (add/delete/reorder/rename) — a "page" in Panel mode is a "chapter" underneath, just relabeled on the frontend, so Panel mode needed zero new backend commands beyond the two export ones. Word counts are computed on the frontend (via Tiptap's `getText()`) and passed in rather than re-derived from Markdown source in Rust. `kind` is `#[serde(default)]` so projects saved before Panel mode existed keep loading as Prose. Entirely separate from Notes and Library; a project is never a note or a library item.
- Panel mode's canvas (`components/studio/PanelPageEditor.tsx`) is Excalidraw — already a dependency for the Canvas feature, so this added zero new npm packages. Panel-layout templates and the page-bounds guide rectangle are built as raw `.excalidraw`-schema element objects by hand (`rectElement` in that file) rather than via any Excalidraw conversion helper, specifically to avoid depending on a utility function whose exact export/signature varies by version — the base element shape is the stable, long-documented file format instead. Panel mode exports the current page as PNG, or the whole project as a multi-page PDF (`exportPanelPdf` in `StudioEditor.tsx`) by sequentially switching through every page, screenshotting each with `html2pdf.js`'s `.toCanvas()` step, and assembling the results using html2pdf.js's own documented `html2pdf__page-break` convention — no PDF-manipulation library needed. This is the newest, least-proven part of Inkwell (the page-flip settle delay is a fixed timeout, not an actual "finished painting" signal); the single-page PNG export is the simpler fallback.
