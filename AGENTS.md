...
- `studio.rs` — Inkwell's backend: writing projects and their chapters, all in `.xuro/studio.json` (same one-JSON-store shape as `manga_online.rs`). Chapter content is Markdown, matching the Notes editor's own content model, so exporting a chapter is just handing that string to a file. Word counts are computed on the frontend (via Tiptap's `getText()`) and passed in rather than re-derived from Markdown source in Rust — parsing formatting syntax to count words would double-count things like `**bold**` markers. Entirely separate from Notes and Library; a project is never a note or a library item. Phase 1 (prose only) — a Konva-based "panel mode" for manga/manhwa page layouts is a planned future addition to this same module/tab, not built yet.
