...
- `<vault>/.xuro/` — app data: `todos.json`, `bookmarks.json`, `locks.json` (salted+hashed PINs, never plaintext), `assets/` (pasted images).
...
- `search.rs` — case-insensitive title+content search, title hits ranked first. Excludes locked/protected notes entirely via `locks::is_protected` — a search result preview would defeat a PIN lock.
...
- `studio.rs` — Inkwell's backend: writing projects and their chapters, all in `.xuro/studio.json` (same one-JSON-store shape as `manga_online.rs`). **`kind` (Prose/Panel) lives on each `Chapter`, not the project** — a project can freely mix Tiptap chapters (`content` is Markdown) and Excalidraw pages (`content` is a serialized scene) in any order, since one project locked to one kind turned out to be the wrong shape once Panel mode existed alongside Prose. `read()` migrates pre-0.1.9 saves, where `kind` lived on the project: every chapter in an old save unconditionally inherits that project's old kind, since chapters never had their own `kind` field before. Word counts are computed on the frontend (via Tiptap's `getText()`) and passed in rather than re-derived from Markdown source in Rust. Entirely separate from Notes and Library; a project is never a note or a library item.
- Panel mode's canvas (`components/studio/PanelPageEditor.tsx`) is Excalidraw — already a dependency for the Canvas feature, so this added zero new npm packages. Panel-layout templates and the page-bounds guide rectangle (a fixed-id element, `xuro-page-bounds`, so a theme switch can find and recolor it) are built as raw `.excalidraw`-schema element objects by hand (`rectElement`) rather than via any Excalidraw conversion helper, to avoid depending on a utility function whose exact export/signature varies by version. `<Excalidraw theme={uiTheme}>` follows Xuro's own dark/light class by default the same way `CanvasEditor.tsx` does, but can be flipped independently via a `Footer`-rendered toggle button (Excalidraw's own theme is UI chrome only, separate from `pageTheme`, which is the page's own background/panel-border color). `--color-primary` (and its darker/darkest/light variants) are overridden to Xuro's own `--invert`/`--active` tokens, scoped to `.xuro-panel-editor .excalidraw` — without this Excalidraw's active-tool highlight is its own default purple, which clashed with the rest of the app. The `<MainMenu>` keeps Excalidraw's genuinely useful default items (`SaveAsImage`, `Export`, `ClearCanvas`, `ChangeCanvasBackground`) but replaces the items that link to Excalidraw's own docs/Discord/socials with Xuro's own GitHub link; Excalidraw's separate Help ("?") dialog isn't officially overridable, so it's hidden via a scoped CSS rule instead of left showing the wrong project's branding. Panel mode exports the current page as PNG, or every Panel page in the project as a multi-page PDF (`exportPanelPdf` in `StudioEditor.tsx`, Prose chapters skipped) by sequentially switching through each page, screenshotting it with `html2pdf.js`'s `.toCanvas()` step, and assembling the results via html2pdf.js's documented `html2pdf__page-break` convention. This is the newest, least-proven part of Inkwell (the page-flip settle delay is a fixed timeout, not an actual "finished painting" signal); the single-page PNG export is the simpler fallback.
- A report of stray duplicate toolbar rows accumulating in `StudioEditor.tsx` (one per unique Prose chapter visited) hasn't been reproduced directly, but the pattern (structural swap between very different subtrees — Suspense/Excalidraw, Tiptap, or an empty state — depending on the active chapter's kind) matches known WebView2/Chromium stale-compositor-layer bugs. As a defensive measure, the toolbar and body are now keyed and torn down together as a single atomic block per chapter switch (`key={chapter?.id ?? "empty"}` on their shared parent) rather than as two independently-reconciled keyed siblings. Not confirmed as the actual root cause.
- `tauri.conf.json`'s main window sets `"dragDropEnabled": false`. Tauri v2 defaults this to `true`, which makes Tauri intercept OS-level file drag-and-drop *before* it reaches the webview — silently breaking any in-page HTML5 drag-and-drop, including Excalidraw's image tool. Confirmed nothing else in the app depends on Tauri's native drag-drop event API before disabling it.
...
- `todos.rs` / `boards.rs` / `bookmarks.rs` — JSON stores in `.xuro/`. `todos.rs`'s `in_progress` field (Kanban) is fully independent of `done` (the flat Tasks list's checkbox) — `toggle` never touches it, only `set_status` (the Kanban drag) does. Every `Todo` also carries a `board_id` (`boards.rs`'s `DEFAULT_BOARD_ID` for anything saved before boards existed); `boards.rs` owns board CRUD and reassigns a deleted board's todos onto the oldest remaining board rather than losing them.
...
- `stores/` — zustand: `vault` (tree, view, theme, recents; `setView`/`toggleExpanded`/`expandTo` all gate through `locks`), `tabs` (open note tabs; active = derived from `vault.view`), `todos`, `boards` (Kanban boards; `activeBoardId` selects which board `KanbanPage` filters todos by), `bookmarks`, `library` (saved items + search results/view mode for `LibraryPage`), `onlineManga` (Manga tab's Online Manga hub: browse filters/results, genres, follows, reading progress, history, bookmarks, downloads — entirely separate from `library`'s state), `studio` (Inkwell projects/chapters), `locks` (also owns the shared PIN dialog's open/close state), `ui`
- `components/` — by feature: `layout/`, `tree/`, `editor/`, `todos/`, `bookmarks/`, `daily/`, `tags/`, `kanban/`, `canvas/` (lazy-loaded — Excalidraw is heavy), `locks/`, `otp/` (third-party, used as pulled), `palette/`, `settings/`, `welcome/`, `ui/`. `library/online/` holds the Online Manga UI (`OnlineMangaHub`, `MangaDetailsPanel`, `MangaReader`) — `LibraryPage` only ever renders it behind the Manga tab's "Online Manga" sub-toggle, never for Books. `studio/` holds Inkwell (`StudioPage`, `StudioEditor`, `PanelPageEditor`) — `PanelPageEditor` is lazy-loaded the same way `canvas/` is, for the same reason (it mounts Excalidraw).
