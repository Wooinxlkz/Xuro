...
- `locks.rs` — PIN locks on notes/folders: salted SHA-256 hash, never plaintext. `is_protected()` is ancestor-aware (locking a folder protects everything inside it) — every full-vault scan (search, tags, backlinks) must pass entries through it.
