...
This folder is a **Xuro** vault: a plain-Markdown knowledge base. You can read
...
- `.xuro/` — app data managed by Xuro: `todos.json`, `bookmarks.json`, tag
  registries, and `assets/` (pasted images). Safe to read; edit only if you
  follow the shapes below.
- `CLAUDE.md` / `AGENTS.md` — this guide (generated by Xuro if missing; your
  edits are kept).
...
- A note may begin with a YAML frontmatter block delimited by `---`.
  **Preserve it** when editing; Xuro renders it as an editable properties
...
- Images reference vault-relative paths under `.xuro/assets/`.
...
- Xuro picks up external changes when its window regains focus.

## App data shapes (`.xuro/`)
...
/// how to work with the vault. Only known layout phrases from Xuro's previous
