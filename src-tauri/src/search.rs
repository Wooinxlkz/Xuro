...
fn walk(
    ...
    for entry in entries.flatten() {
        ...
        if locks::is_protected(root, &rel) {
            // Locked notes are invisible to search entirely — a snippet
            // would defeat the point of locking it.
            continue;
        }
        let title = path
            .file_stem()
...
mod tests {
    ...
    #[test]
    fn locked_notes_are_invisible_to_search() {
        let dir = tempdir().unwrap();
        ensure_layout(dir.path()).unwrap();
        let secret = create_note(dir.path(), "", "Secret plans").unwrap();
        write_note(dir.path(), &secret, "the launch codes are hidden here").unwrap();
        crate::locks::set_pin(dir.path(), &secret, "1234").unwrap();

        assert!(search_notes(dir.path(), "launch codes", 10).unwrap().is_empty());
        assert!(search_notes(dir.path(), "Secret plans", 10).unwrap().is_empty());
    }

    #[test]
    fn notes_inside_a_locked_folder_are_invisible_too() {
        let dir = tempdir().unwrap();
        ensure_layout(dir.path()).unwrap();
        fs::create_dir_all(notes_root(dir.path()).join("Private")).unwrap();
        crate::locks::set_pin(dir.path(), "Private", "1234").unwrap();
        let nested = create_note(dir.path(), "Private", "Diary").unwrap();
        write_note(dir.path(), &nested, "dear diary, today I").unwrap();

        assert!(search_notes(dir.path(), "dear diary", 10).unwrap().is_empty());
    }
}
