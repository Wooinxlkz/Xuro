...
pub const OPEN_EVENT: &str = "xuro:quick-capture-open";
...
/// Xuro window is deliberately left untouched so capture works over any app.
pub fn show(app: &AppHandle) -> AppResult<()> {
    ...
    // window before unhiding Xuro so repeated captures only reveal the panel.
    #[cfg(target_os = "macos")]
    if !main_was_focused {
...
/// Close capture without promoting Xuro's main window when capture was
...
pub fn close(app: &AppHandle) -> AppResult<()> {
    ...
    if !main_was_focused {
        ...
        // hidden so opening Xuro normally still restores the workspace.
