...
pub fn rename_entry(root: &Path, rel: &str, new_name: &str) -> AppResult<String> {
    ...
    let target = if path.is_dir() {
        ...
        // Preserve the original extension (.md, .excalidraw, …) — this used
        // to hardcode "md", which would have silently renamed a canvas
        // file's extension to .md on every rename.
        let ext = path.extension().and_then(|e| e.to_str());
        available_path(&dir, &name, ext)
    };
    fs::rename(&path, &target)?;
    let next = rel_of(root, &target)?;
    backlinks::rewrite_links(root, rel, &next)?;
    pins::remap(root, rel, &next)?;
    locks::remap(root, rel, &next)?;
...
pub fn move_entry(root: &Path, rel: &str, target_dir_rel: &str) -> AppResult<String> {
    ...
    let target = if path.is_dir() {
        ...
        let stem = path
            ...
            .to_string_lossy()
            .to_string();
        // Same fix as rename_entry: keep the file's real extension instead
        // of hardcoding "md".
        let ext = path.extension().and_then(|e| e.to_str());
        available_path(&target_dir, &stem, ext)
    };
    fs::rename(&path, &target)?;
    let next = rel_of(root, &target)?;
    backlinks::rewrite_links(root, rel, &next)?;
...
mod tests {
    ...
    #[test]
    fn rename_and_move_preserve_non_markdown_extensions() {
        let dir = setup();
        create_folder(dir.path(), "", "Canvases").unwrap();
        let path = dir.path().join("Canvases/Sketch.excalidraw");
        fs::write(&path, "{}").unwrap();

        let renamed = rename_entry(dir.path(), "Canvases/Sketch.excalidraw", "Diagram").unwrap();
        assert_eq!(renamed, "Canvases/Diagram.excalidraw");

        let other_folder = create_folder(dir.path(), "", "Archive").unwrap();
        let moved = move_entry(dir.path(), &renamed, &other_folder).unwrap();
        assert_eq!(moved, "Archive/Diagram.excalidraw");
    }
