...
fn rewrite_assets(
    ...
    for captures in image.captures_iter(markdown) {
        ...
        if is_remote(href) || href.starts_with("xuro-asset:") {
            continue;
        }
        ...
        objects.entry(hash.clone()).or_insert(bytes);
        replacements.push((found.start(), found.end(), format!("xuro-asset:{hash}")));
    }
...
mod tests {
    ...
    fn rewrites_and_deduplicates_local_images() {
        ...
        let markdown = rewrite_assets(
            dir.path(),
            "![one](.xuro/assets/pixel.png) ![two](.xuro/assets/pixel.png)",
            &mut objects,
            &mut descriptors,
        )
        .unwrap();
        ...
        assert_eq!(markdown.matches("xuro-asset:").count(), 2);
    }
