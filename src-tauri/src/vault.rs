...
pub const DATA_DIR: &str = ".xuro";
pub const ASSETS_DIR: &str = ".xuro/assets";
/// Default landing folder for new notes created with no folder selected.
/// Purely a UX convention — notes can still live anywhere in the vault root.
pub const DEFAULT_NOTES_DIR: &str = "Notes";
const ROOT_LAYOUT_MARKER: &str = ".xuro/root-notes-layout";
const LEGACY_MIGRATION_DIR: &str = ".xuro/legacy-notes-layout";
...
pub fn ensure_layout(root: &Path) -> AppResult<Vec<LayoutMigration>> {
    ...
    fs::create_dir_all(root.join(DEFAULT_NOTES_DIR))?;
...
/// Resolve a vault-relative note path while rejecting traversal and Xuro's
/// reserved root data.
...
fn scan_dir(root: &Path, dir: &Path) -> AppResult<Vec<TreeNode>> {
    ...
    for entry in fs::read_dir(dir)? {
        ...
        } else if path.extension().is_some_and(|e| e == "md" || e == "excalidraw") {
            let stem = path
                .file_stem()
                .map(|s| s.to_string_lossy().to_string())
                .unwrap_or(name);
            let kind = if path.extension().is_some_and(|e| e == "excalidraw") {
                NodeKind::Canvas
            } else {
                NodeKind::Note
            };
...
mod tests {
    ...
    fn layout_and_scan() {
        ...
        assert!(dir.path().join(DEFAULT_NOTES_DIR).is_dir());
        ...
        assert_eq!(tree[1].name, DEFAULT_NOTES_DIR);
        assert_eq!(tree[1].kind, NodeKind::Folder);
        assert_eq!(tree[2].name, "a note");
    }

    #[test]
    fn scan_tree_recognizes_canvas_files() {
        let dir = tempdir().unwrap();
        ensure_layout(dir.path()).unwrap();
        let notes = notes_root(dir.path());
        fs::write(notes.join("Sketch.excalidraw"), "{}").unwrap();
        fs::write(notes.join("Note.md"), "hi").unwrap();

        let tree = scan_tree(dir.path()).unwrap();
        let sketch = tree.iter().find(|n| n.name == "Sketch").unwrap();
        assert_eq!(sketch.kind, NodeKind::Canvas);
        assert_eq!(sketch.rel, "Sketch.excalidraw");
        let note = tree.iter().find(|n| n.name == "Note").unwrap();
    ...
    fn hidden_entries_skipped() {
        ...
        assert_eq!(tree[0].name, DEFAULT_NOTES_DIR);
    }

    #[test]
    fn migrates_an_existing_xuro_vault_to_the_root() {
        let dir = tempdir().unwrap();
    ...
    fn reserved_root_files_are_not_notes() {
        ...
        assert_eq!(tree[0].name, DEFAULT_NOTES_DIR);
        assert_eq!(tree[1].rel, "Note.md");
        assert!(resolve_rel(dir.path(), "AGENTS.md").is_err());
        assert!(resolve_rel(dir.path(), ".xuro/todos.json").is_err());
    }
}
