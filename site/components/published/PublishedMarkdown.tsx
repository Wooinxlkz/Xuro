...
function isSafeLink(href: string | undefined): boolean {
  ...
  return href.startsWith("#") || href.startsWith("xuro-page:") || /^(https?:|mailto:)/i.test(href);
}
...
function PublishedLink({
  ...
  const pagePath = href?.startsWith("xuro-page:")
    ? href.slice("xuro-page:".length)
    : null;
  const safeHref =
    pagePath === null
      ? href
  ...
  return (
    <div className="published-note">
      ...
      <ReactMarkdown
        ...
        urlTransform={(url) =>
          typeof url === "string" &&
          (url.startsWith("xuro-asset:") || url.startsWith("xuro-page:"))
            ? url
        ...
        components={{
          ...
          img: ({ alt, src }) => {
            const hash = typeof src === "string" && src.startsWith("xuro-asset:")
              ? src.slice("xuro-asset:".length)
              : null;
