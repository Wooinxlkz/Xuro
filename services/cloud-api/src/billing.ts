...
export async function beginCheckout(request: Request, env: Env): Promise<Response> {
  ...
  if (input.token) {
    ...
    if (!handoff) {
      throw new BillingError(401, "billing_link_expired", "Open pricing from Xuro again to continue.");
    }
...
export async function dodoWebhook(
  ...
  if (verified.payload.business_id !== env.DODO_BUSINESS_ID) {
    throw new BillingError(400, "invalid_webhook", "The webhook business does not match Xuro.");
  }
...
async function syncSubscription(
  ...
  if (!userId) throw new BillingError(409, "billing_user_missing", "The subscription has no Xuro user.");
...
async function subscriptionUserId(
  ...
  const metadataUser = subscription.metadata?.xuro_user_id;
  if (metadataUser) {
