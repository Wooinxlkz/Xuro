...
describe("Dodo webhook verification", () => {
  test("accepts a correctly signed Standard Webhooks payload", async () => {
    const rawSecret = new TextEncoder().encode("xuro-webhook-test-secret");
    const secret = `whsec_${base64(rawSecret)}`;
    const id = "msg_xuro_test";
...
describe("Dodo checkout session", () => {
  test("uses minimal address collection and the Xuro success page", async () => {
    ...
    try {
      const env = {
        ...
        PUBLIC_SITE_ORIGIN: "https://usexuro.app/",
      } as Env;
      ...
      expect(requestBody?.return_url).toBe("https://usexuro.app/checkout/success");
