...
describe("publishing identifiers", () => {
  ...
  test("hashes content deterministically", async () => {
    expect(await sha256("Xuro")).toBe(
      "e850ce29353d2816a45e92098d4e4b5003c55ff095b5e94bfcffb69ccfbb53e8",
    );
  });
...
describe("email OTP authentication", () => {
  ...
  test("sends login codes from the configured Xuro sender", async () => {
    ...
    expect(message.from).toEqual({ email: "no-reply@usexuro.app", name: "Xuro" });
    expect(message.to).toBe("person@example.com");
    expect(message.subject).toBe("Your Xuro sign-in code");
    expect(message.text).toContain("123456");
    expect(message.text).toContain("expires in 5 minutes");
  });

  test("sends the welcome message from the configured Xuro sender", async () => {
    const messages: EmailMessageBuilder[] = [];
    const EMAIL = {
      async send(next: EmailMessageBuilder) {
        messages.push(next);
    ...
    expect(message.from).toEqual({
      email: "no-reply@usexuro.app",
      name: "saurabh from xuro",
    });
    expect(message.to).toBe("person@example.com");
    expect(message.subject).toBe("welcome to xuro");
    expect(message.text).toContain("hey person,");
    expect(message.text).toContain("second brain");
    expect(message.text).toContain("https://usexuro.app/download");
    expect(message.html).toContain("prefers-color-scheme: dark");
    expect(message.html).toContain("https://usexuro.app/apple-icon.png");
  });

  test("derives a safe welcome name from the email address", () => {
    expect(welcomeName("john.doe@example.com")).toBe("John");
    expect(welcomeName("SAURABH+xuro@example.com")).toBe("Saurabh");
