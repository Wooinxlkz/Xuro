...
- **Graph view** — a visual map of how your notes connect through links, grouped by folder like a workflow diagram: each folder is outlined (a colored, dashed border, no fill) and can be collapsed or expanded with a click, right-clicking a note or folder shows its path, dates, size, and note count (plus a Rename action), right-clicking empty canvas offers a "New note" shortcut, and a filter box dims anything not matching what you type.
- **Library** — a personal books/novels/manga shelf with list, grid, and bento views. Search and add from free public catalogs (Open Library for books and novels, Jikan for manga, both filtered for explicit content), or upload your own PDF/EPUB/CBZ/CBR file — uploads are copied into a `Library/` folder inside your vault, so they're backed up and synced with everything else. A search-added item with no file yet can have one attached directly via "Attach file", without creating a duplicate entry. Uploaded PDFs open in a real in-app reader — fullscreen, fit-to-width, click-to-jump page input, keyboard shortcuts, page-by-page or scroll-through-every-page reading, and an open-in-default-app fallback; other formats currently open in your system's default viewer.
- **Online Manga** — inside the Manga tab only, a separate "Online Manga" mode alongside your local Manga Library. Browse a large, constantly-updating catalog (search, genres, status, popularity, latest updates, and sorting), filtered to English, Spanish, Arabic, or Japanese and always excluding explicit/adult content. Open and read chapters right in Xuro with a reader that tracks your progress, resumes where you left off, and lets you jump straight to the next/previous chapter or a bookmark. Follow manga to get a "new chapter" indicator without needing an account, keep a reading history, star favorites, and download chapters for fully offline reading — stored inside your vault's existing Library folder, but always kept clearly separate from your local Manga Library.
...
- A local, timestamped debug log (frontend errors, unhandled promise rejections, Rust panics) is kept for troubleshooting, viewable and clearable from Settings → Debug — nothing in it is sent anywhere.
