...
- **Library** — a personal books/manga shelf with list, grid, and bento views. Search and add from free public catalogs (Open Library for books, Jikan for manga, both filtered for explicit content), or upload your own PDF/EPUB/CBZ/CBR file — uploads are copied into a `Library/` folder inside your vault, so they're backed up and synced with everything else. Uploaded PDFs open in a real in-app reader (page navigation, zoom); other formats currently open in your system's default viewer.
